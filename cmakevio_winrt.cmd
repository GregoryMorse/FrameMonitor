REM MSMF video capture broken in OpenCV 3.4.1, enabling VFW and DSHOW
...
REM SET VSPF=D:\Program Files (x86)
SET VSPF=%ProgramFiles(x86)%
IF "%VSVER%"=="15" (SET "msvc_path=%VSPF%\Microsoft Visual Studio\2017\Enterprise\VC\Auxiliary\Build") ELSE (SET "msvc_path=%VSPF%\Microsoft Visual Studio %VSVER%.0")
...
IF "%VSVER%"=="15" (CALL "%VSPF%\Microsoft Visual Studio\2017\Enterprise\Common7\Tools\VsDevCmd.bat" && CD "%CD%") ELSE (CALL "%msvc_path%\Common7\Tools\VsDevCmd.bat")
IF "%VSVER%"=="11" (SET OSVER=8.0) ELSE (IF "%VSVER%"=="12" (SET OSVER=8.1) ELSE (SET OSVER=10))
IF "%VSVER%"=="14" (SET "LIBPATH=%LIBPATH%;%VSPF%\Microsoft Visual Studio 14.0\VC\lib\store\references")
SET sources=../../../sources
SET armtoolchain=%sources%/platforms/winrt/arm.winrt.toolchain.cmake
IF "%VSVER%"=="15" (SET cmakedir="%ProgramFiles%\CMake\bin\cmake.exe") ELSE (SET cmakedir="%VSPF%\CMake\bin\cmake.exe")
